  ...
  return (
    <section>
      <header className="mb-4 flex items-start justify-between gap-3">
        <h1 className="text-[48px] font-extrabold leading-[0.95] tracking-[-0.03em] text-[#101827]">
          Product Gym Floor
        </h1>
        <div className="pt-1 text-right">
          <p className="text-[12px] font-bold uppercase tracking-[0.18em] text-[#2f6df6]">
            Streak ⓘ
          </p>
          <p className="flex items-center justify-end gap-1 text-[30px] font-extrabold leading-none text-[#ff6a00]">
            <Flame className="h-6 w-6 fill-[#ff6a00] text-[#ff6a00]" />12 Days
          </p>
        </div>
      </header>

      <div className="mb-4 flex items-center justify-between rounded-[24px] bg-white px-4 py-3 shadow-md shadow-slate-900/10">
      ...
      <section className="mb-4 rounded-[24px] bg-[#bfd4f3] p-4 shadow-md shadow-blue-900/10">
        <div className="mb-4 flex items-center gap-3">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#2f6df6] text-base font-black text-white">
            {initials || 'PG'}
          </div>
          <div>
            <h2 className="text-[34px] font-black leading-none tracking-[-0.02em] text-[#101827]">{userName}</h2>
            <p className="text-[12px] font-bold uppercase tracking-[0.18em] text-[#2f6df6]">
              Product gym member
            </p>
          </div>
      ...
      <p className="mb-3 text-[40px] font-extrabold leading-[0.95] tracking-[-0.03em] text-[#101827]">
        Practice <span className="align-baseline text-[#2f6df6]">JUNIOR</span>
        <ChevronDown className="mb-1 inline h-6 w-6 align-baseline text-[#2f6df6]" /> PM skills with:
      </p>

      <div className="mb-5 grid h-12 grid-cols-3 rounded-[24px] bg-[#dce3ec] p-1 text-center shadow-[inset_0_1px_1px_rgba(255,255,255,0.5)]">
      ...
      {tab === 'companies' && (
        <div>
          <div className="mb-3 flex items-center justify-between">
            <p className="text-[34px] font-extrabold leading-[1.05] tracking-[-0.02em] text-[#101827]">
...
function CompanyTrackCard({
  ...
  const card = (
    <article className="rounded-[24px] bg-white p-4 shadow-md shadow-slate-900/10">
      <div className="flex items-start gap-3">
        <div className="grid h-12 w-12 place-items-center rounded-[16px] bg-[#f3f5f9] text-[26px] font-black text-[#4385f4]">
          G
        </div>
        <div className="flex-1">
          <h3 className="text-[34px] font-black leading-[1.02] tracking-[-0.02em] text-[#101827]">{track.title}</h3>
          <p className="text-[14px] font-semibold text-[#64748b]">Focus: {track.description ?? 'Product sense'}</p>
          <div className="mt-2 flex items-center gap-4 text-[12px] font-bold uppercase tracking-[0.18em] text-[#94a3b8]">
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <p className="text-[12px] font-bold uppercase tracking-[0.18em] text-[#2563eb]">Resume</p>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="rounded-[24px] bg-white p-4 text-center text-sm font-semibold text-[#94a3b8] shadow-md shadow-slate-900/10">
      {message}
    </div>
  );
}
...
function StatPill({
  ...
  return (
    <div className="rounded-[16px] bg-[#e9edf3] px-2 py-3">
      <div className="mb-1 flex justify-center">{icon}</div>
      <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#7f8ba0]">{title}</p>
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-[20px] px-2 py-2 text-[12px] font-bold uppercase tracking-[0.12em] ${
        active
          ? 'bg-white text-[#2563eb] shadow-sm shadow-slate-900/10'
          : 'text-[#64748b]/90'
      }`}
    >
      {label}
