...
function parsePhoneFromProfile(profile: ProfileValues) {
  const matchedCountry = COUNTRY_OPTIONS.find((option) => option.code === profile.phone_country);
  ...
  if (!national && existingE164.startsWith('+')) {
    const byDial = COUNTRY_OPTIONS.find((option) => existingE164.startsWith(option.dialCode));
    if (byDial) {
...
export default function SettingsScreen({ email, profile, status, error }: { email: string; profile: ProfileValues; status?: string; error?: string }) {
  ...
  const initialValues = useMemo(
    ...
    [email, parsedPhone.phone_country, parsedPhone.phone_dial_code, parsedPhone.phone_national, profile.first_name, profile.last_name, profile.username]
  );
    ...
    <MobileScreen>
      <section className="mx-auto w-full max-w-[361px] space-y-4">
        <header className="flex items-center gap-3">
          ...
          <h1 className="text-[24px] font-bold tracking-[-0.6px] text-[#0f172b]">Settings</h1>
        </header>

        <section className="rounded-[16px] border border-[#dbeafe] bg-white p-3">
          <p className="mb-3 text-[12px] font-bold tracking-[-0.3px] text-[#0f172b]">Account Preferences</p>
          <form action={updateAccountPreferences} className="space-y-3">
            ...
            <PhoneField
              ...
              onNationalChange={(value) => setFormValues((prev) => ({ ...prev, phone_national: value }))}
            />

            <Field label="Email" name="email" value={formValues.email} onChange={(value) => setFormValues((prev) => ({ ...prev, email: value }))} type="email" required />
        ...
        <section className="rounded-[16px] border border-[#dbeafe] bg-white p-3">
          <p className="mb-3 text-[12px] font-bold tracking-[-0.3px] text-[#0f172b]">Security</p>
          <form action={changePassword} className="space-y-3">
            <Field label="Current password" name="currentPassword" defaultValue="" type="password" required minLength={8} />
            <Field label="New password" name="password" defaultValue="" type="password" required minLength={8} />
            <Field label="Confirm password" name="passwordConfirm" defaultValue="" type="password" required minLength={8} />
            <div className="flex justify-end pt-1">
              <button type="submit" className="h-[43px] rounded-[12px] bg-[#2563eb] px-10 text-[10px] font-black uppercase tracking-[1px] text-white">CHANGE PASSWORD</button>
            </div>
          </form>
        </section>
        ...
        <button onClick={() => setOpenModal('deactivation')} className="h-[64px] w-full rounded-[16px] border border-[#ff6467] bg-white p-3 text-left">
          <p className="text-[16px] font-bold tracking-[-0.4px] text-[#e7000b]">Deactivate Account</p>
          <p className="text-[12px] font-medium text-[#ff6467]">Wipe all progress permanently.</p>
        </button>
        ...
        <ConfirmModal open={openModal === 'deactivation'} title="Deactivate Account?" description="This action is permanent and cannot be undone." onCancel={() => setOpenModal(null)}>
          <form action={logout}>
            <button className="h-[43px] w-full rounded-[12px] bg-[#e7000b] text-[10px] font-black uppercase tracking-[1px] text-white">Confirm Deactivation</button>
          </form>
        </ConfirmModal>
      </section>
    </MobileScreen>
...
function PhoneField({
  ...
  return (
    <div>
      <p className="mb-1 text-[9px] font-black uppercase tracking-[1px] text-[#94a3b8]">Phone</p>
      <div className="flex h-[43px] w-full items-center overflow-hidden rounded-[12px] border border-[#e2e8f0] bg-[#f8fafc]">
        <label className="flex h-full min-w-[124px] items-center border-r border-[#dbeafe] pl-3 pr-2 text-[14px] font-medium text-[var(--profile-title-color)]">
          <span className="mr-1">{COUNTRY_OPTIONS.find((option) => option.code === phoneCountry)?.flag ?? '🌍'}</span>
          <select
            name="phone_country"
            value={phoneCountry}
      ...
      <input type="hidden" name="phone" value={`${phoneDialCode}${phoneNational.replace(/\s+/g, '')}`} />
    </div>
  );
}

function Field({ label, className = '', value, defaultValue, type = 'text', name, onChange, readOnly, required, minLength, maxLength, pattern, title }: { label: string; className?: string; value?: string; defaultValue?: string; type?: string; name?: string; onChange?: (value: string) => void; readOnly?: boolean; required?: boolean; minLength?: number; maxLength?: number; pattern?: string; title?: string }) {
  return (
    <div className={className}>
      <p className="mb-1 text-[9px] font-black uppercase tracking-[1px] text-[#94a3b8]">{label}</p>
      <input
        type={type}
        name={name}
        value={value}
