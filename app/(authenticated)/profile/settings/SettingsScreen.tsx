...
export default function SettingsScreen({ email, profile, status, error }: { email: string; profile: ProfileValues; status?: string; error?: string }) {
  const initialValues = useMemo(
    ...
    [email, profile.first_name, profile.last_name, profile.phone, profile.username]
  );
      ...
      <section className="mx-auto w-full max-w-[361px] space-y-4">
        ...
        <section className="rounded-[16px] border border-[#dbeafe] bg-white p-3">
          ...
          <form action={changePassword} className="space-y-3">
            <Field label="New password" name="password" defaultValue="" type="password" required minLength={6} />
            <Field label="Confirm password" name="passwordConfirm" defaultValue="" type="password" required minLength={6} />
            <div className="flex justify-end pt-1">
              <button type="submit" className="h-[43px] w-[140px] rounded-[12px] bg-[#2563eb] text-[10px] font-black uppercase tracking-[1px] text-white">Change password</button>
            </div>
          </form>
        </section>
