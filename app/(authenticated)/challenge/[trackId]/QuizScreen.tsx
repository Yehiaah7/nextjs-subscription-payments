...
export default function QuizScreen({ challengeId }: { challengeId: string }) {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      ...
      const firstPending = normalizedQuiz.questions.findIndex((question) => !nextStates[question.id]?.isSolved);
  ...
  return (
    <section className="mx-auto flex w-full max-w-[361px] flex-col gap-4 pb-4 text-text">
      ...
      <section className="w-full rounded-2xl bg-white p-3 shadow-[0_1px_3px_0_rgba(0,0,0,0.2)]">
        ...
        {currentState.isSolved ? <p className="mt-2 text-xs font-semibold text-green-700">Solved • You can continue</p> : null}
      </section>
      ...
      {showCorrectFeedback ? (
        <div className="rounded-2xl bg-green-100 p-3 text-sm text-green-900">
          ...
          <p>{currentQuestion.explanation ?? 'Great work. You solved this step.'}</p>
        </div>
      ) : null}
